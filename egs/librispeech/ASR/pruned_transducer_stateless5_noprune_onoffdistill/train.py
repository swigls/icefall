...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        loss_off, loss_on, loss_r, loss_out = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
        ...
        loss_off_is_finite = torch.isfinite(loss_off)
        loss_on_is_finite = torch.isfinite(loss_on)
        loss_r_is_finite = torch.isfinite(loss_r)
        loss_out_is_finite = torch.isfinite(loss_out)
        is_finite = loss_off_is_finite & loss_on_is_finite \
            & loss_r_is_finite & loss_out_is_finite 
        if not torch.all(is_finite):
            ...
            loss_off = loss_off[loss_off_is_finite]
            loss_on = loss_on[loss_on_is_finite]
            loss_r = loss_r[loss_r_is_finite]
            loss_out = loss_out[loss_out_is_finite]
            ...
            if torch.all(~loss_off_is_finite) \
                or torch.all(~loss_on_is_finite) \
                or torch.all(~loss_r_is_finite) \
                or torch.all(~loss_out_is_finite):
                raise ValueError(
                    "There are too many utterances in this batch "
                    "leading to inf or nan losses."
                )

        loss_off = loss_off.sum()
        loss_on = loss_on.sum()
        loss_r = loss_r.sum()
        ...
        '''
        loss = (
            params.loss_off_scale * loss_off
            + params.loss_on_scale * loss_on
            + params.loss_r_scale * loss_r
            + params.loss_out_scale * loss_out
        )
    ...
    info["loss_off"] = loss_off.detach().cpu().item()
    info["loss_on"] = loss_on.detach().cpu().item()
    info["loss_r"] = loss_r.detach().cpu().item()
    info["loss_out"] = loss_out.detach().cpu().item()
