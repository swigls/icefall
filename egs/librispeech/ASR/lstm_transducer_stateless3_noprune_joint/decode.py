...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="""Lambda coefficient for joint streaming search.
        (only used for modified_beam_search_joint)
        """,
    )
...
def decode_one_batch(
    ...
    if params.decoding_method == "fast_beam_search":
        ...
        params.decoding_method == "greedy_search_joint"
        and params.max_sym_per_frame == 1
    ):
        hyp_tokens = greedy_search_batch(
            model=model,
        ...
        for hyp in sp.decode(hyp_tokens):
            hyps.append(hyp.split())
    elif params.decoding_method == "modified_beam_search_joint":
        hyp_tokens = modified_beam_search(
            model=model,
            encoder_out=encoder_out,
            encoder_out_lens=encoder_out_lens,
        ...
        for i in range(batch_size):
            ...
            feature_i = feature[i:i + 1, :feature_lens[i]]
            # fmt: on
            ...
            elif params.decoding_method == "greedy_search_joint":
                hyp = greedy_search(
                    model=model,
                    encoder_out=encoder_out_i,
                    max_sym_per_frame=params.max_sym_per_frame,
            ...
            elif params.decoding_method == "beam_search_joint":
                hyp = beam_search(
                    model=model,
                    encoder_out=encoder_out_i,
                    beam=params.beam_size,
    ...
    if params.decoding_method == "greedy_search" \
        or params.decoding_method == 'greedy_search_joint':
