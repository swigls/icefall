...
class ConformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if rel_pos:
            ...
            self.self_attn = PositionMultiheadAttention(d_model, nhead, dropout=0.0)
...
class PositionalEncoding(RelPositionalEncoding):
    def forward(self, x: torch.Tensor, left_context: int = 0) -> Tuple[Tensor, Tensor]:
        self.extend_pe(x, left_context)
        x = x * self.xscale
        x_size_1 = x.size(1) + left_context
        ...
        return self.dropout(x + pos_emb[:, - x.size(1):]), self.dropout(pos_emb)
...
class PositionMultiheadAttention(nn.Module):
    ...
    def multi_head_attention_forward(
        ...
        matrix_a = torch.matmul(q, k)  # (batch, head, time1, time2)
        '''
        ...
        '''
        attn_output_weights = matrix_a * scaling  # (batch, head, time1, time2)
...
class RelPositionMultiheadAttention(nn.Module):
    ...
    def multi_head_attention_forward(
        ...
        #'''
        q_with_bias_u = (q).transpose(
            1, 2
        )  # (batch, head, time1, d_k)

        q_with_bias_v = (q).transpose(
            1, 2
        )
        # compute attention score
        # first compute matrix a and matrix c
