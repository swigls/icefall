...
def greedy_search(
    ...
    if model.encoder_dim != model.decoder_dim:
        encoder_out = model.audio_embedding(encoder_out)  # (1, T, D)
    ...
    while t < T and sym_per_utt < max_sym_per_utt:
        ...
        y_in = torch.Tensor(hyp).unsqueeze(1)  # (1, u)
        y_in = y_in.to(torch.int64).to(device)  # (1, u)
        u = y_in.shape[1]
        if torch.jit.is_tracing():
            # This is for exporting to PNNX via ONNX
            embedding_out = model.decoder.embedding(y_in)
        else:
