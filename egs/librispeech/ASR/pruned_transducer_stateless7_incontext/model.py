...
class Transducer(nn.Module):
    ...
    def forward(
        ...
        mask = (torch.triu(torch.ones(T+U, T+U, device=device))==1).permute(0, 1)  # (T+U, T+U)

        # Length mask for x and y
        batch_mask_x = (torch.arange(0, T, device=device) < x_lens.view(B, 1)).type(torch.bool)  # (B, T)
        print(y_lens, y_lens+1)
        batch_mask_y = (torch.arange(0, U, device=device) < (y_lens+1).view(B, 1)).type(torch.bool)  # (B, U)
        ...
        loss = loss_all * batch_mask_y  # (B,U)
        loss = loss.sum()
        return loss, loss
